...
def patch(existing_cls):
    def decorator(new_cls):
        if isinstance(new_cls, type):  # Patching classes
            assert isinstance(existing_cls, type), (
                f"Can't patch {existing_cls} with {new_cls}"
            )
            for attr, obj in new_cls.__dict__.items():
                if attr in [
                    ...
                    continue
                assert isinstance(obj, type(record)), (
                    f"Can't patch attribute {attr} of {new_cls} to be {obj}"
                )
                setattr(existing_cls, attr, obj)
        elif isinstance(new_cls, type(record)):  # Patching functions
            assert isinstance(existing_cls, type(record)), (
                f"Can't patch {existing_cls} with {new_cls}"
            )
...
def parse_flags():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to composite some elements",
    )
    parser.add_argument(
        "--disable_gpu",
        action="store_true",
        default=False,
        help="Whether to disable use of the GPU",
    )
    parser.add_argument(
        "--show_composited_layer_borders",
        action="store_true",
        default=False,
        help="Whether to visually indicate composited layer borders",
    )
    parser.add_argument(
        "--force_cross_origin_iframes",
        action="store_true",
        default=False,
        help="Whether to treat all iframes as cross-origin",
    )
    parser.add_argument(
        "--assert_layout_clean",
        action="store_true",
        default=False,
        help="Assert layout is clean once complete",
    )
    parser.add_argument(
        "--print_invalidation_dependencies",
        action="store_true",
        default=False,
        help="Whether to print out all invalidation dependencies",
    )
    parser.add_argument(
        "--trace",
        action="store_true",
        default=False,
        help="Whether to output a browser.trace file",
    )
